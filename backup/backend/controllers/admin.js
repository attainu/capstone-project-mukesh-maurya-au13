...
exports.announcementPage = async (req, res) => {
  try {
    ...
    const createAnnouncement = await Announce.create({ announcement, name, designation });
...
exports.userPhoto = upload.single("photo");
  exports.placements = async (req, res) => {
    ...
    res.status(201).json({ status: 'success', message: 'Added successfully', newPlacement})
  } catch(error) {
    res.status(400)
